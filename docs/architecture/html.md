...
- Hosts trim chrome via context keys `html.hideFluidPreset`, `html.hidePreviewToolbar`, and `html.hideZoomControls`, or pass options to `registerHtmlPreviewChrome(ctx, { initialPreset?, hideFluidPreset?, hidePreviewToolbar?, hideZoomControls? })` (e.g. email with a product top bar sets `hidePreviewToolbar: true`). Options from every plugin activation are **merged** before the first registration; context keys apply on every call even when preview commands are already registered.
