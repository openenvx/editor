...
- Hosts trim chrome via context keys `html.hideFluidPreset`, `html.hidePreviewToolbar`, and `html.hideZoomControls`, or pass options to `registerHtmlPreviewChrome(ctx, { initialPreset?, hideFluidPreset?, hideZoomControls? })`. Options from every plugin activation are **merged** before the first registration; context keys apply on every call even when preview commands are already registered. When `layout.topBar` is on (canvas/email product top bar), the preview toolbar contribution uses `when: '!workbench.topBar && !html.hidePreviewToolbar'` so the overlay top-center bar stays hidden.
