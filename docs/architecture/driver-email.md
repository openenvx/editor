...
- `EmailBlocksPlugin` — layer definitions, `email.*` block commands, **Blocks** activity command (opens sheet) + **Elements** (primitives) sidebar, `EmailEditorPane` for `page.layout === 'email'`. **Top bar** (`EmailTopBar`) hosts Editor / HTML / Preview mode switch, undo/redo, device presets, save, and file actions; device/zoom via shared `registerHtmlPreviewChrome()` (`HtmlPreviewChromeService`). Hosts should use `DEFAULT_EMAIL_LAYOUT` (`editorToolbars: false`). Artboard design width 640px — desktop frame 720px with slim body chrome, mobile 390px; `email.root` centers content at editable `maxWidth`, default 600
