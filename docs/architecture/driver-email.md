...
- `EmailBlocksPlugin` - layer definitions, `email.*` block commands (including `email.pasteFromClipboard` - formatted clipboard HTML/plain → blocks in Editor mode), **Blocks** activity command (opens sheet) + **Elements** (primitives) sidebar, `EmailEditorPane` for `page.layout === 'email'`. **Bottom insert bar** (`EmailToolbarContribution` → workbench `EditorChrome` `bottom-center`: undo/redo, text/layout dropdowns, image insert; gated `email.modeEdit`) when `layout.editorToolbars: true`. Optional **top bar** (`EmailBlocksPlugin({ topBar: true })` → `EmailTopBarContribution` / `TopBarBuilder`) hosts Editor / HTML / Preview mode switch, undo/redo, device presets, save, and file actions; device/zoom via shared `registerHtmlPreviewChrome()` (`HtmlPreviewChromeService`). Hosts that want the header use `DEFAULT_EMAIL_LAYOUT` (`topBar: true`, `editorToolbars: false`). `@openenvx/email-studio` `EmailEditor` enables the top bar and overlay toolbars (`topBar: true`, `editorToolbars: true`). Artboard design width 640px - desktop frame 720px with slim body chrome, mobile 390px; `email.root` centers content at editable `maxWidth`, default 600
...
Hosts load `new EmailBlocksPlugin()` (alongside workbench defaults). Demo: `apps/email-demo` (`bun run dev:email`) opts into the product top bar (`EmailBlocksPlugin({ topBar: true })` + `DEFAULT_EMAIL_LAYOUT`). External OSS hosts use **`@openenvx/email-studio`** (`EmailEditor` drop-in; `./runtime` for Node HTML export; public npm) - product top bar + floating bottom insert bar (`editorToolbars: true`; top-center preview toolbar hidden when top bar is on).
