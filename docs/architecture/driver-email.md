...
Hosts load `new EmailBlocksPlugin()` (alongside workbench defaults). Demo: `apps/email-demo` (`bun run dev:email`) opts into the product top bar (`EmailBlocksPlugin({ topBar: true })` + `DEFAULT_EMAIL_LAYOUT`). External OSS hosts use **`@openenvx/email`** (`EmailEditor` drop-in; `./runtime` for Node HTML export; public npm) — product top bar + floating bottom insert bar (`editorToolbars: true`; top-center preview toolbar hidden when top bar is on).
