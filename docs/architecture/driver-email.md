...
Hosts load `new EmailBlocksPlugin()` (alongside workbench defaults). Demo: `apps/email-demo` (`bun run dev:email`) opts into the product top bar (`EmailBlocksPlugin({ topBar: true })` + `DEFAULT_EMAIL_LAYOUT`). External OSS hosts use **`@openenvx/email`** (`EmailEditor` drop-in; `./runtime` for Node HTML export; public npm) — floating artboard toolbars only, no top bar.
