...
Stable contract for building a host/server render API against OpenEnvx templates. Editor and server share `@xmazu/openenvxee-schema` helpers: `extractTemplateManifest`, `applyModifications`, and `validateTemplateNames`.
...
Bannerbear `Modification[]` and inline tokens can coexist on the same scene. Compose `VariablesPlugin` per product (`canvas-studio`, `email-studio`) — not a workbench default.
