...
`openenvx.widget` is a shared scene layer type (`OpenEnvxWidgetLayer` in `@openenvx/studio`), registered by both canvas and HTML plugins (first registration wins). Click binding and isolate lifecycle are owned by the **sandbox** path (`createSandboxExtensionHost` in studio wires canvas click handler + layer type so workbench never imports canvas). Trust model: [extensions.md](extensions.md).
