...
| Email product host bundle | **Done** | `@openenvx/email` (published, public npm) | Drop-in `EmailEditor` + `createEmailScene` + `renderEmailHtml`; minified fat bundle inlines private stack. Chrome: floating bottom insert bar (undo/redo, text/layout dropdowns, image) + top-center preview device/zoom — no product top bar. Headless export: `./runtime`. No plugin/command/sandbox exports. Monorepo HMR: `apps/email-demo`. Bundle smoke: `apps/email-package-demo`. |
