...
| **Pro** | Lives in `@openenvx/canvas-pro`, `@xmazu/openenvxee-studio`, or other closed packages |
...
| Plugin / contribution architecture | **Done** (expand) | `packages/core`, `packages/headless`, `packages/canvas` | Extensible layer types, commands, sidebars, inspectors — not a closed monolith. Primary/secondary view-container locations + content kinds (`tree` / `properties` / `component`) for relocation-ready panes. Host views: optional `ViewContribution.icon` / `group` (accordion headers + section labels), field `description` / `placeholder` / `maxLength`. Inspector `kind: 'segmented'` for short enums (button group); `kind: 'select'` is dropdown-only — see [property-fields.md](docs/architecture/property-fields.md). |
