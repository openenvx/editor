...
| Rich text editing | **Partial** | OSS | TipTap overlay (`canvas-rich-text-editor.tsx`, `html-rich-text-editor.tsx`), `data.html` | On-canvas and HTML-block bold / italic / underline / strike. **No inline color or per-span font family/size yet.** Shrink-to-fit (`autoFit: 'shrink'`) for templated canvas text. |
| HTML block layout editor | **Partial** | OSS | `packages/html/src/editor/html-editor-pane.tsx`, `block-dnd.ts`, `html.flex` / `html.grid` | Visual block tree with drag reorder; Flex/Grid hold flat children (no slot entities). Only root/flex/grid accept children. Selection toolbar, double-click text edit. |
