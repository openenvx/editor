...
| Template policy / frozen layers | **Partial** | Schema `writeMode` / `allowedDataKeys` / `templatePolicy.frozenLayers`, `@openenvx/core` | Workbench enforces `writeMode` + key allowlists; `SceneStore` replays baked `frozenLayers` on finalize. Call `withFrozenLayerSnapshots` when publishing templates. `scene.apply` / agents are not fully gated yet. Brand kits deferred. |
