...
| Declarative embed plugin panels | **Partial** | Pro + protocol | `@xmazu/openenvxee-plugin-protocol`, mappers in `@openenvx/headless`, `PluginPanelPlugin` (`packages/workbench/src/plugin-panel/`) | External JSX/`h` trees compile to fluent builders (inspector + chrome). Internal plugins keep OOP contribution classes + builders. Trust boundary: [Plugin-boundaries.md](Plugin-boundaries.md). Hardening: `allowManifest` default off, bind prefix reject, command allowlists, origin-checked transport. Demo: canvas-demo `/?embed=1` + `/embed-parent.html`. Separate from QuickJS sandbox extensions. |
| Sandbox extensions (plugin + widget) | **Partial** | Pro + protocol | `createSandboxExtensionPlugin` (`@xmazu/openenvxee-studio`), workbench sandbox runtime, `openenvx.widget` canvas layer | Figma-shaped: sandbox **plugin** = user-run tool + `showUI` iframe; sandbox **widget** = on-canvas object + per-layer isolate + `syncedState` (local until CRDT). QuickJS Worker, capability gates, UI duplex, toast `notify`. React OK in `showUI` only. Widget JSX face + multiplayer syncedState + marketplace deferred. See [Plugin-boundaries.md](Plugin-boundaries.md). |
