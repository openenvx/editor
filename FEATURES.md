...
| Dynamic templates / data binding | **Done** | OSS + Pro UI | `packages/schema/src/template.ts` (`extractTemplateManifest`, `applyModifications`, `validateTemplateNames`); canvas `applyModificationsWithTextFit`; canvas-pro Template data panel; contract: [template-api-contract.md](docs/architecture/template-api-contract.md) | Bannerbear-style modifications by unique layer `name` (`text`, `imageUrl`, `color`, `fontFamily`, `fontSize`, `hidden`). Shrink-to-fit text **or** remasure text box height to injected copy; image fit/focal point at render time. |
