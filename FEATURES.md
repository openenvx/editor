...
| Sandbox extensions (plugin + widget) | **Partial** | Pro + protocol | `createSandboxExtensionHost` (`@openenvx/studio`), `SandboxExtensionHost` + `mountSandboxExtensions`, `@openenvx/widget-sdk` + `@openenvx/elements`, `openenvx.widget` layer | Figma lifecycle: widgets author against widget-sdk (`defineCanvasComponent` / `ExtensionManifest`); elements supply tags; source is pushed into a QuickJS Worker (one isolate per extension). Document state = host `data.values`; isolate expands to `RenderNode`. Handler-ID events + `data.handlers`. Host maps to editable `data.children`. Demos: canvas-demo seating / save-the-date; html-demo countdown / RSVP. See [widget-bridge.md](docs/architecture/widget-bridge.md). |
