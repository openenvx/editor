...
| Rich text editing | **Partial** | OSS | TipTap overlay (`canvas-rich-text-editor.tsx`, `html-rich-text-editor.tsx`), `data.html` | On-canvas and HTML/email-block bubble menu: block type (paragraph / lists / quote / code), link, bold / italic / underline / strike / inline code / color, and alignment. HTML/email inline edit: Enter at end of a top-level paragraph inserts a sibling text block; Backspace in an empty block removes it; ArrowUp/Down at doc edges move the caret to the previous/next text block. Blocks can hide sections via `BlockConfig.richTextToolbar` / `childRichTextToolbar` (`blockType`, `link`, `code`, `align` - e.g. Snapvelo hero). Shrink-to-fit (`autoFit: 'shrink'`) for fixed-box templated text; otherwise typography/html edits remasure `transform.height` to content. Curved/plain SVG export still strips marks to plain text. |
| HTML block layout editor | **Partial** | OSS | `packages/html/src/editor/html-editor-pane.tsx`, `HtmlToolbarContribution`, `HtmlPreviewChromeService`, `block-dnd.ts`, `html.flex` / `html.grid`, `html.hero` | Visual block tree with drag reorder via the selection-pill **Move** handle (not whole-block grab); Layers panel reorder unchanged. Flex/Grid hold flat children. Composite blocks use named `data.slots` (atomic in Layers tree). Selection toolbar (duplicate/delete + **Replace image** via `AssetService` when present - pill, surface drop, slot click), click-to-edit text (Enter at paragraph end inserts a sibling `html.text`; Backspace on an empty block removes it; arrows cross text-block boundaries), Delete/Backspace (via `scene.deleteLayer`; Page root protected). Device/zoom via workbench overlay toolbar contributions (`top-center`). Artboard click selects the page root (`*.root`) for page props (no floating selection pill on the root); stage padding / Escape clears selection. See `docs/architecture/html-editor-surfaces.md`. Targets simple web pages + product templates (e.g. SnapVelo, WeselneMomenty). **Email-client HTML** lives in `@openenvx/driver-email`, not here. |
| Email block editor (React-Email) | **Partial** | OSS | `packages/driver-email/` (`EmailBlocksPlugin`, `EmailEditorPane`, `EmailTopBarContribution`, `renderEmailDocument`) | `page.layout === 'email'`. Live block editing reuses html block machinery; export via `@react-email/render` (table-based, inline styles). Editor shows a centered content column on the page body (`email.root` background + body padding; nested sections often supply a white card) with a **top bar** (Editor / HTML / Preview modes, undo/redo, device presets, save) - Preview swaps the artboard for an iframe of `renderEmailDocument` HTML; **HTML** fills the pane with a read-only CodeMirror source view (pretty-printed export markup, line numbers, syntax highlight; device presets hidden). No DnD in HTML or Preview. Selection chrome is outline-only (no wrap padding) so edit spacing stays aligned with export. Document font is Inter via react-email `Font` (`@font-face` + Arial/Helvetica fallback); edit artboard uses the same stack (not workbench Geist). Layout primitives map 1:1 to react-email: `email.row` → `<Row>`, `email.column` → `<Column>` (not a synthetic columns flex). Artboard design width 640px; desktop frame 720px (slim body chrome); mobile 390px; root `maxWidth` default 600 (editable) and body `paddingX`/`paddingY` editable. Delete/Backspace removes the selection (Email root protected); selection chrome + context menu also delete. **Formatted paste:** Cmd/Ctrl+V in Editor mode (not while inline-editing text) converts clipboard HTML/plain into blocks (`email.heading`, `email.text`, lists, inline links, `email.image`, `email.divider`) inserted after the selection - root paste wraps in a new `email.section`. Inline text keyboard: Enter at the end of a paragraph inserts a sibling `email.text`; Backspace in an empty block removes it (not the last child of a container); ArrowUp/Down at document edges move between editable text blocks. **Templates** activity opens a left gallery sheet (collections → templates drill-down; click loads a full starter Scene via `api.loadScene` - Barebones / Activation first). **Blocks** activity opens a left gallery sheet (search + group filters; patterns via `defineEmailPattern` in `blocks/patterns/` - `email.header`, `email.articleWithImage` as containers whose Elements appear as nested Layers); **Elements** for primitives (root / section / row / column / heading / text / button / image / image link / divider / spacer). Modern clients first; Outlook-desktop VML hardening deferred. **Template variables:** `scene.variables` catalog + `{{{key}}}` tokens in layer data; bottom-bar insert dropdown; secondary **Variables** tab; inline chips + `{{` autocomplete in TipTap (missing `sample` warns in editor); `renderEmailHtml(scene, { variables })` for headless fill. **Out of scope for now:** ESP sending/delivery. |
...
| Plugin / contribution architecture | **Done** (expand) | `packages/core`, `packages/canvas` | Extensible layer types, commands, sidebars, inspectors - not a closed monolith. Primary/secondary view-container locations + content kinds (`tree` / `list` / `properties` / `component`) for relocation-ready panes. Host views: optional `ViewContribution.icon` / `group` (accordion headers + section labels), field `description` / `placeholder` / `maxLength`. Inspector `kind: 'segmented'` for short enums (button group); `kind: 'select'` is dropdown-only. Property rows/blocks/input groups support layout `when` (context keys + `$` property paths) - see [property-fields.md](docs/architecture/property-fields.md). |
| Secondary sidebar + layout chrome | **Done** | Pro UI (`packages/workbench`, `ViewLocationService`, `WorkbenchLayout`) | Right sidebar Radix tabs; hide activity bar / primary / secondary independently; move whole containers via header menu; drag-reorder within each side; optional `WorkbenchLayoutStore` persistence. Per–view-container sidebar headers via `SidebarHeaderContribution` (`containerId` + title / dropdown menu / icon actions). Optional shell **top bar** via `layout.topBar` + composable `TopBarContribution` / `TopBarBuilder` (email + canvas studio enable this; workbench `TopBarRenderer` renders contributed actions). |
...
| HTML product host bundle | **Done** | `@openenvx/html-studio` (published, public npm) | Drop-in `HtmlEditor` + host composition API (`WorkbenchShell`, plugins, contributions); Worker-safe `./runtime` (`renderBlockDocument` + overrides, no TipTap/DnD). Minified fat bundle inlines private stack. Monorepo HMR: `apps/html-demo` (html + workbench). Bundle smoke: `apps/html-package-demo`. Product event-page blocks live in host apps (e.g. Snapvelo), not editor-core. |
| Email product host bundle | **Done** | `@openenvx/email-studio` (published, public npm) | Drop-in `EmailEditor` + `createEmailScene` + `renderEmailHtml`; minified fat bundle inlines private stack. Chrome: product top bar (Editor / HTML / Preview, undo/redo, device presets, save) + floating bottom insert bar (text/layout/image). Headless export: `./runtime`. No plugin/command/sandbox exports. Monorepo HMR: `apps/email-demo`. Bundle smoke: `apps/email-package-demo`. |
| Canvas product host bundle | **Done** | `@openenvx/canvas-studio` (published, public npm) | Drop-in `CanvasEditor` + `createCanvasScene`; minified fat bundle inlines private stack. Headless scene factory: `./runtime`. No raster export. Monorepo HMR: `apps/canvas-demo` (canvas + workbench). Bundle smoke: `apps/canvas-package-demo`. |
