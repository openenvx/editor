...
| HTML block layout editor | **Partial** | OSS | `packages/html/src/editor/html-editor-pane.tsx`, `html-device-toolbar.tsx`, `block-dnd.ts`, `html.flex` / `html.grid`, `html.hero` | Visual block tree with drag reorder; Flex/Grid hold flat children. Composite blocks use named `data.slots` (atomic in Layers tree). Selection toolbar, double-click text edit. Device preview toolbar (mobile/tablet/desktop/fit + zoom Auto). The page frame itself is not selectable on canvas — pick **Page** in Layers for its props. **Out of scope:** email-client HTML (Outlook/MSO tables, merge tags, ESP). HTML studio targets simple web pages + product templates (e.g. SnapVelo, WeselneMomenty), not Unlayer-style email builders. |
