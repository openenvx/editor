...
| Grouping | **Done** | OSS | `packages/canvas/src/commands/canvas-group-commands.ts`, `packages/canvas/src/scene/group-layers.ts`, container drag in `canvas-stage-layer.tsx` | Nested groups; layers tree shows children. Click selects the hit layer; selected group/widget drags as a unit (including when grabbing children); child transforms stay relative to the parent. `openenvx.widget` uses the same container path. |
