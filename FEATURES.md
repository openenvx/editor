...
| Declarative embed plugin panels | **Partial** | Pro + protocol | `@xmazu/openenvxee-protocol`, `@xmazu/openenvxee-elements/panel`, `@xmazu/openenvxee-widget-sdk` `renderPanelTree`, mappers in `@openenvx/headless`, `EmbedPanelHost` + `mountEmbedPanel` | External Preact panel trees compile to fluent builders (inspector + chrome). Static `ExtensionManifest` declares contributions; runtime bodies arrive as `render` messages. Mounted off PluginManager via narrow host surfaces. Trust boundary: [Plugin-boundaries.md](Plugin-boundaries.md). Demo: canvas-demo `/?embed=1` + `/embed-parent.html`. Separate from QuickJS sandbox extensions. |
| Sandbox extensions (plugin + widget) | **Partial** | Pro + protocol | `createSandboxExtensionHost` (`@xmazu/openenvxee-studio`), `SandboxExtensionHost` + `mountSandboxExtensions`, `@xmazu/openenvxee-widget-sdk` + `@xmazu/openenvxee-elements`, `openenvx.widget` layer | Figma lifecycle: widgets author against widget-sdk (`defineCanvasComponent` / `ExtensionManifest`); elements supply tags; source is pushed into a QuickJS Worker (one isolate per extension). Document state = host `data.values`; isolate expands to `RenderNode`. Handler-ID events + `data.handlers`. Host maps to editable `data.children`. Demos: canvas-demo seating / save-the-date; html-demo countdown / RSVP. See [widget-bridge.md](docs/architecture/widget-bridge.md). |
