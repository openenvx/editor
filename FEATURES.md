...
| Declarative embed plugin panels | **Partial** | Pro + protocol | `@xmazu/openenvxee-plugin-protocol`, mappers in `@openenvx/headless`, `EmbedPanelHost` + `mountEmbedPanel` (`packages/workbench/src/plugin-panel/`) | External JSX/`h` trees compile to fluent builders (inspector + chrome). Internal plugins keep OOP contribution classes + builders. Mounted off PluginManager via narrow `EmbedPanelHostSurface`. Trust boundary: [Plugin-boundaries.md](Plugin-boundaries.md). Hardening: `allowManifest` default off, bind prefix reject, command allowlists, origin-checked transport. Demo: canvas-demo `/?embed=1` + `/embed-parent.html`. Separate from QuickJS sandbox extensions. |
| Sandbox extensions (plugin + widget) | **Partial** | Pro + protocol | `createSandboxExtensionHost` (`@xmazu/openenvxee-studio`), `SandboxExtensionHost` + `mountSandboxExtensions`, workbench sandbox runtime, `openenvx.widget` canvas layer | Figma-shaped: sandbox **plugin** = user-run tool + non-modal floating `showUI` panel (iframe); sandbox **widget** = on-canvas object + per-layer isolate + `syncedState` (local until CRDT). QuickJS Worker, capability gates, UI duplex, toast `notify`. React OK in `showUI` only. Mounted off PluginManager via narrow `SandboxHostSurface`. Widget JSX face + multiplayer syncedState + marketplace deferred. See [Plugin-boundaries.md](Plugin-boundaries.md). |
...
| Secondary sidebar + layout chrome | **Done** | Pro UI (`packages/workbench`, `ViewLocationService`, `WorkbenchLayout`) | Right sidebar Radix tabs; hide activity bar / primary / secondary independently; move whole containers via header menu; drag-reorder within each side; optional `WorkbenchLayoutStore` persistence. |
