...
| Document version history | **Partial** | Pro UI + host provider | `VersionHistoryProvider` (`@openenvx/headless`), `VersionHistoryPlugin` + panel (`@openenvx/workbench`), `versionHistory.restore` | Figma-style list UI + restore. Host implements `listVersions` / `loadVersion`. Create / rename / delete not in the provider contract yet. |
| Declarative embed plugin panels | **Partial** | Pro + protocol | `@openenvx/protocol`, `@openenvx/elements/panel`, `@openenvx/widget-sdk` `renderPanelTree`, mappers in `@openenvx/headless`, `EmbedPanelHost` + `mountEmbedPanel` | External Preact panel trees compile to fluent builders (inspector + chrome). Static `ExtensionManifest` declares contributions; runtime bodies arrive as `render` messages. Mounted off PluginManager via narrow host surfaces. Trust boundary: [Plugin-boundaries.md](Plugin-boundaries.md). Demo: canvas-demo `/?embed=1` + `/embed-parent.html`. Separate from QuickJS sandbox extensions. |
| Sandbox extensions (plugin + widget) | **Partial** | Pro + protocol | `createSandboxExtensionHost` (`@openenvx/studio`), `SandboxExtensionHost` + `mountSandboxExtensions`, `@openenvx/widget-sdk` + `@openenvx/elements`, `openenvx.widget` layer | Figma lifecycle: widgets author against widget-sdk (`defineCanvasComponent` / `defineHtmlComponent` + `ExtensionManifest`); elements supply tags; source is pushed into a QuickJS Worker (one isolate per extension). Document state = host `data.values`; isolate expands to `RenderNode`. Handler-ID events + `data.handlers` on **canvas and HTML** faces. Host maps to editable `data.children` (HTML: `bind` + nested `values` paths, face parts not listed in Layers). Demos: canvas-demo seating / save-the-date; html-demo countdown / RSVP / **wedding menu** (`menu.widget.tsx`). See [widget-bridge.md](docs/architecture/widget-bridge.md). |
...
| Rich text editing | **Partial** | OSS | TipTap overlay (`canvas-rich-text-editor.tsx`, `html-rich-text-editor.tsx`), `data.html` | On-canvas and HTML/email-block bubble menu: block type (paragraph / lists / quote / code), link, bold / italic / underline / strike / inline code / color, and alignment. Shrink-to-fit (`autoFit: 'shrink'`) for fixed-box templated text; otherwise typography/html edits remasure `transform.height` to content. Curved/plain SVG export still strips marks to plain text. |
| HTML block layout editor | **Partial** | OSS | `packages/html/src/editor/html-editor-pane.tsx`, `html-device-toolbar.tsx`, `block-dnd.ts`, `html.flex` / `html.grid`, `html.hero` | Visual block tree with drag reorder; Flex/Grid hold flat children. Composite blocks use named `data.slots` (atomic in Layers tree). Selection toolbar, double-click text edit, Delete/Backspace (via `scene.deleteLayer`; Page root protected). Device preview toolbar (mobile/tablet/desktop/fit + zoom Auto). The page frame itself is not selectable on canvas — pick **Page** in Layers for its props. Targets simple web pages + product templates (e.g. SnapVelo, WeselneMomenty). **Email-client HTML** lives in `@openenvx/driver-email`, not here. |
| Email block editor (React-Email) | **Partial** | OSS | `packages/driver-email/` (`EmailBlocksPlugin`, `EmailEditorPane`, `renderEmailDocument`) | `page.layout === 'email'`. Live block editing reuses html block machinery; export via `@react-email/render` (table-based, inline styles). Editor shows a centered ~600px white card on the page body (standard email layout) with device preview toolbar (mobile/tablet/desktop/fit + zoom). Desktop/tablet frames stay ~680px (slim body chrome around the card) so the email reads large; mobile is 390px. Content padding on the white frame is editable (default 32px). Delete/Backspace removes the selection (Email root protected); selection chrome + context menu also delete. **Blocks** activity opens a left gallery sheet (search + group filters; patterns via `defineEmailPattern` in `blocks/patterns/` — `email.header`, `email.articleWithImage` as containers whose Elements appear as nested Layers); **Elements** for primitives (root / section / columns / heading / text / button / image / divider / spacer). Modern clients first; Outlook-desktop VML hardening deferred. **Out of scope for now:** ESP sending, merge tags. |
