...
| Email product host bundle | **Done** | `@openenvx/email` (published, public npm) | Drop-in `EmailEditor` + `createEmailScene` + `renderEmailHtml`; minified fat bundle inlines private stack. Chrome: product top bar (Editor / HTML / Preview, undo/redo, device presets, save) + floating bottom insert bar (text/layout/image). Headless export: `./runtime`. No plugin/command/sandbox exports. Monorepo HMR: `apps/email-demo`. Bundle smoke: `apps/email-package-demo`. |
