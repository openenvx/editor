...
import styles from './pattern-blocks-sheet.module.css';
...
export const EmailPatternBlocksSheet = memo(() => {
  ...
  const open =
    useWorkbenchContextSelector(
      (state) => state.contextKeys?.[EMAIL_BLOCKS_SHEET_OPEN_KEY] === true
    ) ?? false;
  const registry =
  ...
  const setOpen = useCallback(
    (next: boolean) => {
      api
        ...
        ?.setContext(EMAIL_BLOCKS_SHEET_OPEN_KEY, next);
    },
    [api]
  );
  ...
  const handleInsert = useCallback(
    ...
    [executeCommand, registry, scene, selection, setOpen]
  );
    ...
    <Sheet onOpenChange={setOpen} open={open}>
      <SheetContent side="left">
        <SheetHeader className={styles.sheetHeader}>
          ...
          <SheetDescription>
            Insert a predefined email section.
          </SheetDescription>
        </SheetHeader>
...
EmailPatternBlocksSheet.displayName = 'EmailPatternBlocksSheet';
