...
import { testArtboard, testDocument } from './test/canvas-document-fixtures';

describe('canvas layers', () => {
  it('text layer renders richText preview descriptor', () => {
    const layer = new CanvasTextLayer();
    const scene = testDocument([testArtboard({ id: 'p1' })]);
    const created = layer.createDefault('t1', scene.artboards[0]!);
  ...
  it('rect layer renders preview descriptor', () => {
    ...
    const scene = testDocument([testArtboard({ id: 'p1' })]);
    const created = layer.createDefault('r1', scene.artboards[0]!);
    const view = layer.renderPreview({
      isSelected: false,
      layerId: created.id,
      model: layer.getModel(created),
