...
| --- | --- | --- |
| `@openenvx/studio` | npmjs (`registry.npmjs.org`, public) | Scene model, plugin host, workbench controller (`./schema`, `./preview`, `.`, `./react`) |
...
The five public npm packages share **one version** per GitHub Release (`vX.Y.Z`). (`@openenvx/studio` is a subpath of `@openenvx/studio`, not a separate registry package.)
